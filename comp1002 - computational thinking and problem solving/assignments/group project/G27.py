...
def update():
    ...
    if choice == 0:
        ...
        while True:
            ...
            if stype == "Utilities" or stype == "Housing" or stype == "Transportation" or stype == "Others":
                break
            else:
                print("Please enter valid input: ")
        expense_service.append(service)
        expense_price.append(price)
        expense_date.append(dates)
        expense_stype.append(stype)
        expense["Expense"] = expense_service
...
def total_Income():
    ...
    for i in range(len(df)): #to go through each element in the data file
        if (i == (len(df))-2 or i == len(df)-1):
            sum = sum + df.loc[i, ['Amount']]
    print("The Total Income of last year is ", sum)
    return main()
...
\n4) Display percentages of the categories of money spent on necessities\n")
    choice = int(input("Please enter number 1-4:"))
    if choice == 0:
        print("quit")
    elif choice == 1:
        update()
